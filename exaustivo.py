...
def escreverArquivo(nome, tempo, MelhorRamo, MaiorInteresse, CapacidadeMelhorRamo, qtdRamos, sucesso=0):
    try:
        a = open(nome, 'at')
    except:
        ...
        try:
            a.write(f'{tempo};{MelhorRamo};{MaiorInteresse};{CapacidadeMelhorRamo};{qtdRamos};{sucesso}\n')
        except:
...
def solucionaProblemaMochila(lista, C, saida):
    ...
    arquivoResultados = saida + 'resultadosExaustivo.txt'
    print(arquivoResultados)
    if not arquivoExiste(arquivoResultados):
        criarArquivo(arquivoResultados)
    escreverArquivo(arquivoResultados, tempo_total, melhorRamo, maiorInteresse, maiorCapacidade, contador, sucesso)
